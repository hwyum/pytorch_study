import os
import pandas as pd
from sklearn.model_selection import train_test_split
from pathlib import Path
from konlpy.tag import Mecab
import gluonnlp as nlp
import pickle
import itertools
from tokenizer import JamoTokenizer

# data load
data_root = './data/'
data_path = Path(data_root) / 'kor_pair_train.csv'
data_df = pd.read_csv(data_path)[['question1', 'question2', 'is_duplicate']]
data_df.is_duplicate = data_df.is_duplicate.map(lambda x: 1-x)  # is_duplicate 레이블 변경 : 중복이면 1, 중복 아니면 0으로 (현재 노테이션이 헷갈려서...)

# train-validation split
train_df, val_df = train_test_split(data_df, test_size=0.2)

# save train / validation data
train_df.to_csv(Path(data_root) / 'tr_pairs.csv', index=False)
val_df.to_csv(Path(data_root) / 'val_pairs.csv', index=False)

# build vocab
tokenizer = Mecab()
tr_tokenized_q1 = [tokenizer.morphs(q) for q in train_df.question1]
tr_tokenized_q2 = [tokenizer.morphs(q) for q in train_df.question2]

counter = nlp.data.count_tokens(itertools.chain.from_iterable([tokens for tokens in tr_tokenized_q1+tr_tokenized_q2]))
vocab = nlp.Vocab(counter=counter, min_freq=10)

# connecting embedding to vocab
ptr_embedding = nlp.embedding.create('fasttext', source='wiki.ko')
vocab.set_embedding(ptr_embedding)

# save vocab
vocab_path = Path(data_root) / 'word_vocab.pkl'
with open(vocab_path, mode='wb') as io:
    pickle.dump(vocab, io)

# build vocab for jamo tokenization
jamo_tokenizer = JamoTokenizer()
tr_jamotokenized_q1 = [jamo_tokenizer.tokenize(''.join(morphs)) for morphs in tr_tokenized_q1]
tr_jamotokenized_q2 = [jamo_tokenizer.tokenize(''.join(morphs)) for morphs in tr_tokenized_q2]

char_counter = nlp.data.count_tokens(itertools.chain.from_iterable([tokens for tokens in tr_jamotokenized_q1+tr_jamotokenized_q2]))
char_vocab = nlp.Vocab(counter=char_counter)

# save vocab
char_vocab_path = Path(data_root) / 'char_vocab.pkl'
with open(char_vocab_path, mode='wb') as io:
    pickle.dump(char_vocab, io)

# save test data
test_path = Path(data_root) / 'kor_Pair_test.csv'
test_df = pd.read_csv(test_path)[['question1','question2','is_duplicate']]
test_df.is_duplicate = data_df.is_duplicate.map(lambda x: 1-x)
test_df.to_csv(Path(data_root) / 'tst_pairs.csv', index=False)
